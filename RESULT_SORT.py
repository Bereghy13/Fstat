...
def CSV_BTTS_SORT(csv_reader):
    ...
    def BTTS_NO(btts_pred_num, line_count, btts_res_num):
        btts_pred_num += 1
        if int(row["Result - Home Team Goals"]) < 1 or int(row["Result - Away Team Goals"]) < 1:
            line_count += 1
            btts_res_num += 1
            BTTS_LIST.append(
                f'{line_count}. {row["date_GMT"]} - {row["Country"]}: {row["League"]} ==> {row["Home Team"]} - {row["Away Team"]} -'
    ...
    print(f'CSV file sorainak száma: {len(sorted_csv)}')
        ...
        if row["Match Status"] == "complete":
            if BTTS_Avg_Low > int(row["BTTS Average"]):
                if btts_pred_num > 0:
                    ...
                    print(
                        f'Nincs meghatározott esemény a {BTTS_Avg_Low} és a {BTTS_Avg_Low + BTTS_interval} tartományban!')
                    BTTS_interval = 10
                    BTTS_Avg_Low = BTTS_Avg_Low - BTTS_interval
            ...
            match BTTS_Avg_Low:
                ...
                case 50:
                    btts_pred_num, line_count, btts_res_num = BTTS_YES(btts_pred_num, line_count, btts_res_num)
                case 40:
                    btts_pred_num, line_count, btts_res_num = BTTS_NO(btts_pred_num, line_count, btts_res_num)
                case 30:
                    btts_pred_num, line_count, btts_res_num = BTTS_NO(btts_pred_num, line_count, btts_res_num)
                case 20:
                    btts_pred_num, line_count, btts_res_num = BTTS_NO(btts_pred_num, line_count, btts_res_num)
    ...
    print(f'A BTTS események száma: {BTTS_sum}')
